...
- [ ] 1. 시맨틱한 방법으로 변경 html
- [x] 2. 반응형 media query 사용 - grid-template-columns 사용
- [ ] 3. 다크 모드 지원
...
.container {
  display: grid
  grid-template-column: 1fr 1fr 1fr // 여기를 수정 사이즈마다
  }
}
```
...
- [x] 1. 웹사이트가 켜졋을때, input에 자동 포커스 let inputbox = document.querySelector('input); inputbox.focus();
- [x] 1-2 인풋클릭시, 인풋 비우기
- [ ] 2. 데이터 호출시, loading 컴포넌트 보여주기
- [ ] 2-1 호출실패시, 에러컴포너트 보여주기 try.. catch..
- [ ] 3. 검색창 컴포넌트 밑에 히스토리 컴포넌트 만들기
- [ ] 4. localState 에 저장된 검색어를 히스토리 컴포넌트에 뿌리기???
- [ ] 5. 램던 버튼을 하나 생성하여, 특정 api `/api/cats/random50` 호출하는 기능 추가
- [ ] 6. lazy load 구현?
- [ ] 7. image tag
...
json.state === '200' ? json.data : json.message 대충 이런식으로

```JavaScript
  const request = async (url: string) => {try {const result = await fetch(url);
  return result.json();} catch (e) {console.warn(e);}}
  const api = {fetchGif: keyword => {return request(`${API_ENDPOINT}/api/gif/search?q=${keyword}`);     },     fetchGifAll: () => {       return request(`${API_ENDPOINT}/api/gif/all`);     }   };
```
